...
{
    bool bRc = false;
    size_t nPackBufSize = pHead->dwMaxVolumeSize; 
    BYTE* pPackBuf = new BYTE[nPackBufSize];
    size_t nRawBufSize = pHead->dwMaxVolumeSize;
    ...
    for (DWORD i = 0; i< pHead->dwFileCount; ++i)
    {
        ...
        */
        ...
        if ( nPackBufSize < dwPackLen )
        {
            delete[] pPackBuf;
            nPackBufSize = dwPackLen;
            pPackBuf = new BYTE[nPackBufSize];
        }
        ...
        */
        ...
        if ( nRawBufSize < nRawDataSize )
        {
            delete[] pRawBuf;
            nRawBufSize = nRawDataSize;
            pRawBuf = new BYTE[nRawBufSize];
        }
